...
조정이 불가능하지만 내용은 모두 변경 가능하다.

도움말
------

명령과 옵션에서 도움말 인자를 받는다. 그리고 명령에서는 함수
독스트링을 적어 주면 그걸 자동으로 이용한다.
...
:param:이나 :return: 행이 나오는 걸 보고 싶지는 않을 수 있다.

이스케이프 표시 ``\f``\를 쓰면 클릭이 그 위치에서
도움말을 자른다.
...
표기 방식을 쓸 수 있다. 모든 층위에서 조정이 가능하다.
...
명령들에는 짧은 도움말 문장이 생성된다. 기본적으로 너무 길지만
