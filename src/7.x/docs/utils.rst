...
유니코드 출력을 지원한다. 즉 `click.echo`\만 이용하면
...
클릭 3.0부터는 ``err=True``\만 주면 손쉽게 표준 오류로
출력할 수도 있다. ::
...
:func:`echo`\와 :func:`style`\ 조합을 :func:`secho`\라는
함수 하나로 대신할 수도 있다. ::
...
항상 stdout으로, 그리고 가능하면 페이저를 통해서 출력한다.
...
참고로 이 함수는 stdin이 파이프인 경우에도 항상 터미널에서
읽는다.
...
하는 문자가 ``^C``\와 ``^D``\인데, 각각 키보드 인터럽트와 파일 끝
예외로 바뀐다. 이렇게 하는 이유는 안 그러면 미처 생각을 못 해서
제대로 끝낼 방법이 없는 스크립트를 만들게 되기 십상이기 때문이다.
...
사용자가 키보드의 아무 키나 누를 때까지 실행을 정지하는 게 유용할
...
또는 파일명을 지정해서 그 파일에 대해 편집기를 띄우게 쓸
...
관리자를 열면서 지정한 파일이 자동으로 선택돼 있게 할 수 있다.
...
:func:`open_file` 함수를 통해서도 이용할 수 있다. 다른 파일뿐
...
응용에서 자기 설정 파일을 열고 싶을 때가 자주 있다. 하지만
