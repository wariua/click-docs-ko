...
별칭으로 ``git ci``\를 받도록 ``git``\를 설정할 수 있다. 다른
도구들에서는 자동으로 명령을 줄여서 별칭을 자동으로 찾아내는
것도 지원한다.

클릭에서는 이 기능을 바로 지원하지 않는다. 하지만 :class:`Group`\이나
...
앞서 본 것처럼 매개변수들(옵션과 인자)이 명령 콜백으로 전달된다.
...
모든 매개변수들의 딕셔너리이고 그 딕셔너리에 있는 게 모두
콜백으로 전달된다.
...
호출하는 쪽에서 준 인자들로 다른 명령을 호출하기만 하는 반면
:func:`Context.forward`\는 현재 명령의 인자들을 가져다 채우기까지
...
진행하면서 매개변수 콜백 호출이 일어나는 반만 클릭에서는 패싱
후에 콜백 호출이 일어난다.
...
긴급 표시:
    ...
    모두 비긴급 매개변수에 앞서 평가한다. 그리고 그 안에선
...
반복되는 매개변수:
    옵션이나 인자가 명령행에서 여러 곳에 나눠져서 반복되는
    경우에는 (예를 들어 ``--exclude foo --include baz
    --exclude bar``) 첫 번째 옵션 위치에 따라 콜백이 실행된다.
    예로 든 경우에선 ``exclude``\에 대해 콜백이 실행되며
    두 옵션(``foo`` 및 ``bar``)을 모두 받게 되고, 그 다음에
    ``include``\에 대해 콜백이 실행되며 ``baz``\만 받게 된다.

    참고로 매개변수에서 여러 값을 허용하지 않더라도 클릭에서는
...
2.  문맥 클래스의 이름이 같은
    ...
    명령의 ``context_settings`` 딕셔너리를 통해 바꾸는 게 가장 좋다.

대부분의 경우에 쉬운 방법은 두 번째다. 그렇게 동작 방식을
바꾸고 나면 어디선가 그 나머지 옵션들(이 시점에선 인자라고
봄)을 가져가야 한다. 여기에도 두 가지 방법이 있다.
...
*   모르는 짧은 옵션들이 필요시 일부만 처리되고 재조합될 수도 있다.
    ...
    처리될 것이다.
*   뭘 하려느냐에 따라선 인자 섞어 쓰기를
    (:attr:`~Context.allow_interspersed_args`) 꺼서 원하는 걸 얻을
    수도 있다. 그 동작을 끄면 인자와 옵션이 섞여 있는 걸 파서에서
    허용하지 않게 된다. 상황에 따라 더 나은 결과를 얻을 수도 있다.

하지만 일반적으로 자체 명령과 다른 응용에서 가져온 명령의 옵션과
...
가능하다. :func:`get_current_context` 함수를 쓰면 되며 현재
문맥을 반환해 준다. 문맥에 연결된 객체나 거기 저장된 플래그들에
...
현재 스레드 내에서만 동작한다는 점에 유의하자. 추가로 스레드를
만들면 그 스레드에서는 현재 문맥을 참조할 수 없게 된다.
다른 스레드에서 현재 문맥을 참조할 수 있게 하고 싶다면
문맥 관리자로서 그 스레드 내에서 문맥을 사용해 줘야 한다. ::
...
즉 문맥에서 읽는 것만 가능하며 변경을 수행하는 건 안 된다.
