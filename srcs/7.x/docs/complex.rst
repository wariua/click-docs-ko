...
그래서 Celery와 Django를 함께 사용할 때는 상호작용이 필요하고
서로에 대해 설정해 줘야 하는 두 가지 도구가 존재하게 된다.

어느 상상의 세계에서 두 가지 클릭 명령행 도구가 따로 있다면 한 명령
내에 다른 명령을 포함시켜서 이 문제를 해결할 수 있다. 예를 들면
웹 프레임워크에 메시지 큐 프레임워크용 명령들을 함께 올릴 수 있다.
...
그리고 각 문맥은 부모 문맥에 연결돼 있다. 그래서 어떤 명령이
다른 명령 아래에서 동작할 때 부모 명령의 상태를 건드리지 않을까
염려할 필요 없이 자기 문맥에 정보를 저장할 수 있다.
...
콜백을 호출할 수 있을까? 답은 문맥 자체에서 그에 대한 처리를
해 주는 헬퍼 함수(:meth:`Context.invoke`)를 제공한다는 것이다.
...
이 예시에서 우리는 버전 관리 시스템을 닮은 명령행 도구를 만들려고
한다. 일반적으로 Git 같은 시스템에서는 최상위 명령이 하나 있어서
몇몇 매개변수와 설정을 받으며 추가로 하위 명령들이 있어서 다른
일들을 한다.
...
그리고 이 명령에서는 도구의 상태를 설정하는 매개변수 몇 가지를
받고 싶다.
...
그리고 그 상태 객체를 :attr:`~Context.obj`\로 문맥에 적어 둔다.
이 특별한 속성을 통해 명령에서 그 자식에게 전달해야 하는 것들을
기억시킬 수 있다.
...
멋진 플러그인이 있는데 많은 설정이 필요하기 때문에 설정들을
...
이를 지원하는 게 클릭에 내장된 :func:`make_pass_decorator`
팩토리인데, 객체를 찾는 데코레이터를 생성해 준다. (내부적으로
:meth:`Context.find_object`\를 호출한다.) 이번 경우에선
가장 가까이 있는 ``Repo`` 객체를 찾고 싶은 것이므로 다음처럼
데코레이터를 만들자.
...
이제 ``pass_obj`` 대신 ``pass_repo``\를 사용해 주면 항상 저장소를
받게 된다.
...
하면서 원래 거기 있던 객체를 대체하게 될 수도 있다. 또한 이
경우에 외부 명령을 실행하지 않더라도 여전히 명령이 실행
가능하다. 한편 이 방식이 동작하려면 객체 타입에 인자를
안 받는 생성자가 있어야 한다.
