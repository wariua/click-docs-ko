...
여럿 있다. 클릭에서는 옵션과 :ref:`위치가 정해진 인자
<arguments>`\를 구별한다.
...
앞에 대시가 붙은 가장 긴 인자로 옵션을 **암묵적으로** 가리킬 수 있다.
...
인자를 여러 개 받는 옵션이 가끔 있다. 옵션에서는 개수가 정해진
인자만 지원하며 ``nargs`` 매개변수로 개수를 설정할 수 있다.
그러면 값들이 튜플로 저장된다.
...
보다시피 `nargs`\를 써서 개수를 지정하면 결과 튜플의 항목들이
...
예를 들어 ``git commit -m foo -m bar``\라고 하면 커밋 메시지로
``foo``\와 ``bar`` 두 줄을 기록하게 된다. ``multiple`` 플래그로
그렇게 할 수 있다.
...
불리언 플래그란 켜고 끌 수 있는 옵션이다. 옵션을 켜거나 끄기
...
윈도우 방식 매개변수를 쓴다면) 대신 ``;``\으로 매개변수를 나눌 수
있다.
...
불리언 플래그 외에 기능 스위치라는 것도 있다. 같은 매개변수 이름으로
...
때로는 값 목록 중에 고르는 매개변수가 필요하다. 그런 경우
...
어떤 경우에는 명령행에서 줄 수도 있고 안 주면 사용자 입력을
...
기본 질문 문자열이 마음에 들지 않으면 다른 문자열로 물을
수도 있다.
...
문맥 옵션 ``auto_envvar_prefix``\와 ``default_map``\을 통해
...
뭐가 기본값이 되는지 설명하려면 ``show_default``\에 설정하면 된다.
...
위험한 동작에 대해 사용자 확인을 받을 수 있다면 아주 좋을 것이다.
...
매개변수들을 이렇게 조합해서 쓰는 게 꽤 흔하기 때문에 대신
...
클릭에서 유용한 기능으로 일반 매개변수뿐 아니라 환경 변수로부터
매개변수를 받는 동작이 있다. 이를 통해 도구들을 손쉽게
자동화할 수 있다. 예를 들어 설정 파일을 ``--config`` 매개변수로
줄 수도 있지만 더 편한 개발을 위해 ``TOOL_CONFIG=hello.cfg``
...
이 경우 여러 환경 변수들의 목록을 줄 수도 있으며 그러면
첫 번째로 정의돼 있는 변수를 쓴다.
...
해결한다. ``multiple``\이나 ``1`` 아닌 값의 ``nargs``\가 있을 때
클릭에서 :meth:`ParamType.split_envvar_value` 메소드를 호출해서
쪼개기를 수행하게 된다.

모든 타입에서 기본 구현은 공백에서 나누는 것이다. 예외로
:class:`File` 및 :class:`Path` 타입에선 운영 체제의 경로 분리
규칙에 따라 나눈다. 리눅스나 OS X 같은 유닉스 시스템에서는
콜론(``:``)에서 분리가 이뤄지고 윈도우에서는
세미콜론(``;``)에서 이뤄진다.
...
예를 들어 슬래시를 매개변수 ``/``\로 처리하거나 비슷한 뭔가를 하고
...
-   기본 모드(잘라내지 않기 모드)에서는 값이 범위를 벗어나면
    오류를 일으킨다.
-   선택적인 잘라내기 모드에서는 값이 범위를 벗어나면 잘리게
...
경계 어느 쪽에든 ``None``\을 주면 범위 그쪽이 열려 있다는
뜻이다.
